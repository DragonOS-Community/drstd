...
impl MetadataExt for Metadata {
    #[allow(deprecated)]
    fn as_raw_stat(&self) -> &raw::stat {
        // The methods below use dlibc::stat, so they work fine when libc is built with FreeBSD 12 ABI.
        // This method would just return nonsense.
        #[cfg(freebsd12)]
        panic!("as_raw_stat not supported with FreeBSD 12 ABI");
        #[cfg(not(freebsd12))]
        unsafe {
            &*(self.as_inner().as_inner() as *const dlibc::stat as *const raw::stat)
        }
    }
    fn st_dev(&self) -> u64 {
