...
impl UnixListener {
    ...
    ///
    /// [`socket address`]: crate::std::os::unix::net::SocketAddr
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// ```
        ...
        unsafe {
            let inner = Socket::new_raw(dlibc::AF_UNIX, dlibc::SOCK_STREAM)?;
            #[cfg(target_os = "linux")]
            const backlog: dlibc::c_int = -1;
            #[cfg(not(target_os = "linux"))]
            const backlog: dlibc::c_int = 128;
            cvt(dlibc::bind(
                inner.as_raw_fd(),
                &socket_addr.addr as *const _ as *const _,
                socket_addr.len as _,
            ))?;
            cvt(dlibc::listen(inner.as_raw_fd(), backlog))?;
            Ok(UnixListener(inner))
        }
    }
    ...
    ///
    /// [`UnixStream`]: crate::std::os::unix::net::UnixStream
    ///
    /// # Examples
    ///
    /// ```no_run
