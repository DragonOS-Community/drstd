...
use crate::std::fs::{self, OpenOptions, Permissions};
use crate::std::io;
use crate::std::os::unix::io::{AsFd, AsRawFd};
...
pub trait OpenOptionsExt {
    ...
    /// options.write(true);
    /// if cfg!(unix) {
    ///     options.custom_flags(dlibc::O_NOFOLLOW);
    /// }
    /// let file = options.open("foo.txt");
    /// # }
    /// ```
...
/// [`std::env::set_current_dir`][`crate::std::env::set_current_dir`] afterwards.
///
/// # Examples
///
/// ```no_run
