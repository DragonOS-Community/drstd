...
//!
//! [channels]: crate::std::sync::mpsc
//! [`join`]: JoinHandle::join
//! [`Result`]: crate::std::result::Result
//! [`Ok`]: crate::std::result::Result::Ok
//! [`Err`]: crate::std::result::Result::Err
//! [`thread::current`]: current
//! [`thread::Result`]: Result
//! [`unpark`]: Thread::unpark
//! [`thread::park_timeout`]: park_timeout
//! [`Cell`]: crate::std::cell::Cell
//! [`RefCell`]: crate::std::cell::RefCell
//! [`with`]: LocalKey::with
//! [`thread_local!`]: crate::std::thread_local
...
use crate::std::sys::thread as imp;
use crate::std::sys_common::thread;
use crate::std::sys_common::thread_info;
use crate::std::sys_common::thread_parking::Parker;
...
cfg_if::cfg_if! {
    if #[cfg(test)] {
        ...
        #[doc(hidden)]
                pub mod local_impl {
            pub use crate::std::sys::common::thread_local::{thread_local_inner, Key, abort_on_dtor_unwind};
        }
    }
}
...
impl Builder {
        ...
        'scope: 'a,
    {
        ...
        let output_capture = crate::std::io::set_output_capture(None);
        crate::std::io::set_output_capture(output_capture.clone());
        ...
        let main = move || {
            ...
            let try_result = panic::catch_unwind(panic::AssertUnwindSafe(|| {
                crate::std::sys_common::backtrace::__rust_begin_short_backtrace(f)
            }));
...
/// ```
///
/// [`channel`]: crate::std::sync::mpsc
/// [`join`]: JoinHandle::join
/// [`Condvar`]: crate::std::sync::Condvar
/// [`Mutex`]: crate::std::sync::Mutex
pub fn yield_now() {
    ...
    fn new() -> ThreadId {
        ...
        cfg_if::cfg_if! {
            if #[cfg(target_has_atomic = "64")] {
                use crate::std::sync::atomic::{AtomicU64, Ordering::Relaxed};
...
/// the [`Error`](crate::std::error::Error) trait.
///
...
///
/// [`Result`]: crate::std::result::Result
/// [`std::panic::resume_unwind`]: crate::std::panic::resume_unwind
pub type Result<T> = crate::std::result::Result<T, Box<dyn Any + Send + 'static>>;
...
impl<T> JoinHandle<T> {
    ...
    ///
    /// [`Err`]: crate::std::result::Result::Err
    /// [atomic memory orderings]: crate::std::sync::atomic
    ///
    /// # Panics
    ///
