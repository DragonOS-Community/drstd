...
use crate::std::thread::{self, Scope, ThreadId};
...
fn test_named_thread_truncation() {
    ...
    let long_name = crate::std::iter::once("test_named_thread_truncation")
        .chain(crate::std::iter::repeat(" yada").take(100))
        .collect::<String>();
    ...
    let result = Builder::new().name(long_name.clone()).spawn(move || {
        ...
        unsafe {
            dlibc::pthread_getname_np(dlibc::pthread_self(), buf.as_mut_ptr().cast(), buf.len());
        }
