...
pub mod panic_count {
    ...
    // the panic handling in the child created by `dlibc::fork`.
    // Memory allocations performed in a child created with `dlibc::fork` are undefined
    // behavior in most operating systems.
    // Accessing LOCAL_PANIC_COUNT in a child created by `dlibc::fork` would lead to a memory
...
//     crate::std::sys_common::backtrace::__rust_end_short_backtrace(move || {
...
pub const fn begin_panic<M: Any + Send>(msg: M) -> ! {
    ...
    return crate::std::sys_common::backtrace::__rust_end_short_backtrace(move || {
        rust_panic_with_hook(
            &mut PanicPayload::new(msg),
            None,
            loc,
