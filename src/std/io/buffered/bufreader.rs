...
impl<R: ?Sized> BufReader<R> {
    ...
    #[inline]
    pub(in crate::std::io) fn discard_buffer(&mut self) {
...
impl<R: ?Sized + Read> Read for BufReader<R> {
    ...
    fn read_exact(&mut self, buf: &mut [u8]) -> io::Result<()> {
        ...
        crate::std::io::default_read_exact(self, buf)
    }
    ...
    fn read_to_string(&mut self, buf: &mut String) -> io::Result<usize> {
        ...
        if buf.is_empty() {
            ...
            unsafe { crate::std::io::append_to_string(buf, |b| self.read_to_end(b)) }
        } else {
            ...
            let string = crate::std::str::from_utf8(&bytes).map_err(|_| {
                io::const_io_error!(
