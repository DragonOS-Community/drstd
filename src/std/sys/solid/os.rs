...
impl itron::error::ItronError {
    #[inline]
    pub(crate) fn as_io_error(self) -> crate::std::io::Error {
        crate::std::io::Error::from_raw_os_error(self.as_raw())
    }
}
...
pub fn setenv(k: &OsStr, v: &OsStr) -> io::Result<()> {
    run_with_cstr(k.as_bytes(), |k| {
        run_with_cstr(v.as_bytes(), |v| {
            ...
            cvt_env(unsafe { dlibc::setenv(k.as_ptr(), v.as_ptr(), 1) }).map(drop)
        })
    })
}
...
pub fn unsetenv(n: &OsStr) -> io::Result<()> {
    run_with_cstr(n.as_bytes(), |nbuf| {
        let _guard = ENV_LOCK.write();
        cvt_env(unsafe { dlibc::unsetenv(nbuf.as_ptr()) }).map(drop)
    })
}
