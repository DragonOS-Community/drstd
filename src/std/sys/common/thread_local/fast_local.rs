...
use crate::std::sys::thread_local_dtor::register_dtor;
use crate::std::{fmt, mem, panic};
...
pub macro thread_local_inner {
    ...
    (@key $t:ty, const $init:expr) => {{
        ...
        unsafe fn __getit(
            ...
            // just get going.
            if !$crate::std::mem::needs_drop::<$t>() {
                unsafe {
                    return $crate::std::option::Option::Some(&VAL)
                }
            }
            ...
            #[thread_local]
            static STATE: $crate::std::cell::Cell<$crate::std::primitive::u8> = $crate::std::cell::Cell::new(0);

            // Safety: Performs `drop_in_place(ptr as *mut $t)`, and requires
            // all that comes with it.
            unsafe extern "C" fn destroy(ptr: *mut $crate::std::primitive::u8) {
                $crate::std::thread::local_impl::abort_on_dtor_unwind(|| {
                    let old_state = STATE.replace(2);
                    $crate::std::debug_assert_eq!(old_state, 1);
                    // Safety: safety requirement is passed on to caller.
                    unsafe { $crate::std::ptr::drop_in_place(ptr.cast::<$t>()); }
                });
            }
            ...
            unsafe {
                match STATE.get() {
                    ...
                    //   so now.
                    0 => {
                        $crate::std::thread::local_impl::Key::<$t>::register_dtor(
                            $crate::std::ptr::addr_of_mut!(VAL) as *mut $crate::std::primitive::u8,
                            destroy,
                        );
                        STATE.set(1);
                        $crate::std::option::Option::Some(&VAL)
                    }
                    // 1 == the destructor is registered and the value
                    //   is valid, so return the pointer.
                    1 => $crate::std::option::Option::Some(&VAL),
                    // otherwise the destructor has already run, so we
                    // can't give access.
                    _ => $crate::std::option::Option::None,
                }
            }
        }
    ...
    (@key $t:ty, $init:expr) => {
        {
            ...
            unsafe fn __getit(
                ...
                static __KEY: $crate::std::thread::local_impl::Key<$t> =
                    $crate::std::thread::local_impl::Key::<$t>::new();
