...
use crate::std::{mem::MaybeUninit, time::Duration};
...
pub fn dur2reltims(dur: Duration) -> impl Iterator<Item = abi::RELTIM> {
    ...
    crate::std::iter::from_fn(move || {
        if ticks == 0 {
            None
        } else if ticks <= abi::TMAX_RELTIM as u128 {
            Some(crate::std::mem::replace(&mut ticks, 0) as abi::RELTIM)
        } else {
...
fn dur2tmos(dur: Duration) -> impl Iterator<Item = abi::TMO> {
    ...
    crate::std::iter::from_fn(move || {
        if end {
            ...
            Some(crate::std::mem::replace(&mut ticks, 0) as abi::TMO)
        } else {
            ticks -= abi::TMAX_RELTIM as u128;
            Some(abi::TMAX_RELTIM)
        }
