...
use crate::std::mem::{self, MaybeUninit};
use crate::std::os::windows::io::{AsHandle, BorrowedHandle};
...
use crate::std::sys::{c, cvt, Align8};
    ...
    pub fn file_attr(&self) -> io::Result<FileAttr> {
        unsafe {
            ...
            cvt(c::GetFileInformationByHandleEx(
                self.handle.as_raw_handle(),
                c::FileBasicInfo,
                &mut info as *mut _ as *mut dlibc::c_void,
                size as c::DWORD,
            ))?;
...
unsafe fn from_maybe_unaligned<'a>(p: *const u16, len: usize) -> Cow<'a, [u16]> {
    if p.is_aligned() {
        Cow::Borrowed(crate::std::slice::from_raw_parts(p, len))
    } else {
        ...
        *db = mem::zeroed();
        ...
        for c in iter {
            if i >= reparse_target_slice.len() {
                return Err(crate::std::io::const_io_error!(
                    crate::std::io::ErrorKind::InvalidFilename,
