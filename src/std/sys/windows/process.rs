...
use crate::std::env;
use crate::std::env::consts::{EXE_EXTENSION, EXE_SUFFIX};
...
use crate::std::sys::args::{self, Arg};
use crate::std::sys::c;
use crate::std::sys::c::NonZeroDWORD;
use crate::std::sys::cvt;
use crate::std::sys::fs::{File, OpenOptions};
use crate::std::sys::handle::Handle;
use crate::std::sys::path;
...
use dlibc;
use dlibc::{c_void, EXIT_FAILURE, EXIT_SUCCESS};
    ...
    pub fn kill(&mut self) -> io::Result<()> {
        ...
        if result == c::FALSE {
            ...
            if error != c::ERROR_ACCESS_DENIED || self.try_wait().is_err() {
                return Err(crate::std::io::Error::from_raw_os_error(error as i32));
            }
        }
        Ok(())
    }
...
pub struct CommandArgs<'a> {
    iter: crate::std::slice::Iter<'a, Arg>,
}
