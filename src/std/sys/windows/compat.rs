...
macro_rules! ansi_str {
    (sym $ident:ident) => {{ crate::std::sys::compat::const_cstr_from_bytes(concat!(stringify!($ident), "\0").as_bytes()) }};
    ($lit:literal) => {{ crate::std::sys::compat::const_cstr_from_bytes(concat!($lit, "\0").as_bytes()) }};
}
...
pub(crate) const fn const_cstr_from_bytes(bytes: &'static [u8]) -> &'static CStr {
    ...
    unsafe { crate::std::ffi::CStr::from_bytes_with_nul_unchecked(bytes) }
}
...
#[derive(Copy, Clone)]
pub(in crate::std::sys) struct Module(NonNull<c_void>);
impl Module {
...
macro_rules! compat_fn_with_fallback {
    ...
    $(
        ...
        pub mod $symbol {
            ...
            use crate::std::sync::atomic::{AtomicPtr, Ordering};
            use crate::std::sys::compat::Module;
    ...
    $(
        ...
        $(
            pub mod $symbol {
                ...
                use crate::std::mem;
                use crate::std::ptr::{self, NonNull};
                use crate::std::sync::atomic::{AtomicPtr, Ordering};

                pub(in crate::std::sys) static PTR: AtomicPtr<c_void> = AtomicPtr::new(ptr::null_mut());
