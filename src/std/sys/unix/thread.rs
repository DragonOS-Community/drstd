...
use crate::std::io;
use crate::std::mem;
use crate::std::num::NonZeroUsize;
use crate::std::ptr;
use crate::std::sys::{os, stack_overflow};
use crate::std::time::Duration;
use dlibc;

#[cfg(all(target_os = "linux", target_env = "gnu"))]
use crate::std::sys::weak::dlsym;
...
mod zircon {
    ...
    extern "C" {
        pub fn zx_object_set_property(
            handle: zx_handle_t,
            property: u32,
            value: *const dlibc::c_void,
            value_size: dlibc::size_t,
        ) -> zx_status_t;
...
impl Thread {
    ...
    pub unsafe fn new(stack: usize, p: Box<dyn FnOnce()>) -> io::Result<Thread> {
        ...
        assert_eq!(dlibc::pthread_attr_init(&mut attr), 0);
        ...
        if stack > 0 {
            ...
            assert_eq!(
                dlibc::pthread_attr_setstacksize(&mut attr, cmp::max(stack, min_stack_size(&attr))),
                0
            );
        }
        ...
        #[cfg(not(target_os = "espidf"))]
        {
            ...
            match dlibc::pthread_attr_setstacksize(&mut attr, stack_size) {
                0 => {}
                n => {
                    assert_eq!(n, dlibc::EINVAL);
                    ...
                    assert_eq!(dlibc::pthread_attr_setstacksize(&mut attr, stack_size), 0);
                }
            };
        }

        let ret = dlibc::pthread_create(&mut native, &attr, thread_start, p as *mut _);
        // Note: if the thread creation fails and this assert fails, then p will
        // be leaked. However, an alternative design could cause double-free
        // which is clearly worse.
        assert_eq!(dlibc::pthread_attr_destroy(&mut attr), 0);
        ...
        extern "C" fn thread_start(main: *mut dlibc::c_void) -> *mut dlibc::c_void {
            unsafe {
    ...
    pub fn yield_now() {
        let ret = unsafe { dlibc::sched_yield() };
        debug_assert_eq!(ret, 0);
    }

    #[cfg(target_os = "android")]
    pub fn set_name(name: &CStr) {
        const PR_SET_NAME: dlibc::c_int = 15;
        unsafe {
            dlibc::prctl(
                PR_SET_NAME,
                name.as_ptr(),
    ...
    pub fn set_name(name: &CStr) {
        ...
        unsafe {
            ...
            let res = dlibc::pthread_setname_np(dlibc::pthread_self(), name.as_ptr());
    ...
    pub fn set_name(name: &CStr) {
        unsafe {
            dlibc::pthread_set_name_np(dlibc::pthread_self(), name.as_ptr());
        }
    }
    ...
    pub fn set_name(name: &CStr) {
        unsafe {
            ...
            let res = dlibc::pthread_setname_np(name.as_ptr());
            // We have no good way of propagating errors here, but in debug-builds let's check that this actually worked.
            debug_assert_eq!(res, 0);
        }
    }
    ...
    pub fn set_name(name: &CStr) {
        unsafe {
            ...
            let res = dlibc::pthread_setname_np(
                dlibc::pthread_self(),
                cname.as_ptr(),
                name.as_ptr() as *mut dlibc::c_void,
            );
            debug_assert_eq!(res, 0);
        }
    }
    ...
    pub fn set_name(name: &CStr) {
        weak! {
            fn pthread_setname_np(
                dlibc::pthread_t, *const dlibc::c_char
            ) -> dlibc::c_int
        }

        if let Some(f) = pthread_setname_np.get() {
            let res = unsafe { f(dlibc::pthread_self(), name.as_ptr()) };
            debug_assert_eq!(res, 0);
        }
    }
    ...
    pub fn set_name(name: &CStr) {
        ...
        unsafe {
            zx_object_set_property(
                zx_thread_self(),
                ZX_PROP_NAME,
                name.as_ptr() as *const dlibc::c_void,
                name.to_bytes().len(),
            );
        }
    }
    ...
    pub fn set_name(name: &CStr) {
        unsafe {
            let thread_self = dlibc::find_thread(ptr::null_mut());
            dlibc::rename_thread(thread_self, name.as_ptr());
        }
    }
    ...
    pub fn sleep(dur: Duration) {
        ...
        unsafe {
            while secs > 0 || nsecs > 0 {
                let mut ts = dlibc::timespec {
                    tv_sec: cmp::min(dlibc::time_t::MAX as u64, secs) as dlibc::time_t,
                    tv_nsec: nsecs,
                };
                secs -= ts.tv_sec as u64;
                let ts_ptr = &mut ts as *mut _;
                if dlibc::nanosleep(ts_ptr, ts_ptr) == -1 {
                    assert_eq!(os::errno(), dlibc::EINTR);
                    secs += ts.tv_sec as u64;
                    nsecs = ts.tv_nsec;
                } else {
                    nsecs = 0;
    ...
    pub fn id(&self) -> dlibc::pthread_t {
        self.id
    }

    pub fn into_id(self) -> dlibc::pthread_t {
        let id = self.id;
        mem::forget(self);
        id
    }
...
impl Drop for Thread {
    fn drop(&mut self) {
        let ret = unsafe { dlibc::pthread_detach(self.id) };
        debug_assert_eq!(ret, 0);
    }
}
...
fn truncate_cstr<const MAX_WITH_NUL: usize>(cstr: &CStr) -> [dlibc::c_char; MAX_WITH_NUL] {
    ...
    cfg_if::cfg_if! {
        if #[cfg(any(
            ...
            #[cfg(any(target_os = "android", target_os = "linux",target_os = "dragonos",))]
            {
                ...
                let mut set: dlibc::cpu_set_t = unsafe { mem::zeroed() };
                unsafe {
                    if dlibc::sched_getaffinity(0, mem::size_of::<dlibc::cpu_set_t>(), &mut set) == 0 {
                        let count = dlibc::CPU_COUNT(&set) as usize;
            ...
            match unsafe { dlibc::sysconf(dlibc::_SC_NPROCESSORS_ONLN) } {
            ...
            {
                let mut set: dlibc::cpuset_t = unsafe { mem::zeroed() };
                unsafe {
                    if dlibc::cpuset_getaffinity(
                        dlibc::CPU_LEVEL_WHICH,
                        dlibc::CPU_WHICH_PID,
                        -1,
                        mem::size_of::<dlibc::cpuset_t>(),
                        &mut set,
                    ) == 0 {
                        let count = dlibc::CPU_COUNT(&set) as usize;
                        if count > 0 {
            ...
            {
                unsafe {
                    let set = dlibc::_cpuset_create();
                    if !set.is_null() {
                        let mut count: usize = 0;
                        if dlibc::pthread_getaffinity_np(dlibc::pthread_self(), dlibc::_cpuset_size(set), set) == 0 {
                            for i in 0..u64::MAX {
                                match dlibc::_cpuset_isset(i, set) {
                                    -1 => break,
                                    0 => continue,
                                    _ => count = count + 1,
                                }
            ...
            let mut cpus: dlibc::c_uint = 0;
            let mut cpus_size = crate::std::mem::size_of_val(&cpus);

            unsafe {
                cpus = dlibc::sysconf(dlibc::_SC_NPROCESSORS_ONLN) as dlibc::c_uint;
            }

            // Fallback approach in case of errors or no hardware threads.
            if cpus < 1 {
                let mut mib = [dlibc::CTL_HW, dlibc::HW_NCPU, 0, 0];
                let res = unsafe {
                    dlibc::sysctl(
                        mib.as_mut_ptr(),
                        2,
            ...
            unsafe {
                let mut sinfo: dlibc::system_info = crate::std::mem::zeroed();
                let res = dlibc::get_system_info(&mut sinfo);
...
mod cgroups {
    ...
    use crate::std::fs::{try_exists, File};
...
pub mod guard {
    ...
    use dlibc::{mmap as mmap64, mprotect};
    #[cfg(all(target_os = "linux", target_env = "gnu"))]
    use dlibc::{mmap64, mprotect};
    use dlibc::{MAP_ANON, MAP_FAILED, MAP_FIXED, MAP_PRIVATE, PROT_NONE, PROT_READ, PROT_WRITE};

    use crate::std::io;
    use crate::std::ops::Range;
    use crate::std::sync::atomic::{AtomicUsize, Ordering};
    use crate::std::sys::os;
    ...
    unsafe fn get_stack_start() -> Option<*mut dlibc::c_void> {
        ...
        assert_eq!(dlibc::stack_getbounds(&mut current_stack), 0);
        Some(current_stack.ss_sp)
    }

    #[cfg(target_os = "macos")]
    unsafe fn get_stack_start() -> Option<*mut dlibc::c_void> {
        let th = dlibc::pthread_self();
        let stackptr = dlibc::pthread_get_stackaddr_np(th);
        Some(stackptr.map_addr(|addr| addr - dlibc::pthread_get_stacksize_np(th)))
    }
    ...
    unsafe fn get_stack_start() -> Option<*mut dlibc::c_void> {
        let mut ret = None;
        let mut attr: dlibc::pthread_attr_t = crate::std::mem::zeroed();
        #[cfg(target_os = "freebsd")]
        assert_eq!(dlibc::pthread_attr_init(&mut attr), 0);
        #[cfg(target_os = "freebsd")]
        let e = dlibc::pthread_attr_get_np(dlibc::pthread_self(), &mut attr);
        #[cfg(not(target_os = "freebsd"))]
        let e = dlibc::pthread_getattr_np(dlibc::pthread_self(), &mut attr);
        if e == 0 {
    ...
    unsafe fn get_stack_start_aligned() -> Option<*mut dlibc::c_void> {
    ...
    pub unsafe fn current() -> Option<Guard> {
        ...
        if e == 0 {
            let mut guardsize = 0;
            assert_eq!(dlibc::pthread_attr_getguardsize(&attr, &mut guardsize), 0);
            if guardsize == 0 {
...
#[cfg(all(target_os = "linux", target_env = "gnu"))]
fn min_stack_size(attr: *const dlibc::pthread_attr_t) -> usize {
    // We use dlsym to avoid an ELF version dependency on GLIBC_PRIVATE. (#23628)
    // We shouldn't really be using such an internal symbol, but there's currently
    // no other way to account for the TLS size.
    dlsym!(fn __pthread_get_minstack(*const dlibc::pthread_attr_t) -> dlibc::size_t);

    match __pthread_get_minstack.get() {
        None => dlibc::PTHREAD_STACK_MIN,
