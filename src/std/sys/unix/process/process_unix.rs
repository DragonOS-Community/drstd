...
#[cfg(target_os = "linux")]
use crate::std::os::linux::process::PidFd;
...
use dlibc::RTP_ID as pid_t;

#[cfg(not(target_os = "vxworks"))]
use dlibc::{c_int, pid_t};
...
use dlibc::{gid_t, uid_t};

cfg_if::cfg_if! {
    if #[cfg(all(target_os = "nto", target_env = "nto71"))] {
        use crate::std::thread;
        use dlibc::{c_char, posix_spawn_file_actions_t, posix_spawnattr_t};
        ...
        fn get_clock_resolution() -> Duration {
            static MIN_DELAY: LazyLock<Duration, fn() -> Duration> = LazyLock::new(|| {
                let mut mindelay = dlibc::timespec { tv_sec: 0, tv_nsec: 0 };
                if unsafe { dlibc::clock_getres(dlibc::CLOCK_MONOTONIC, &mut mindelay) } == 0
                {
...
impl Command {
    ...
    pub fn output(&mut self) -> io::Result<(ExitStatus, Vec<u8>, Vec<u8>)> {
        ...
        crate::std::sys_common::process::wait_with_output(proc, pipes)
    }
    ...
    unsafe fn do_fork(&mut self) -> Result<pid_t, io::Error> {
        ...
        loop {
            let r = dlibc::fork();
            if r == -1 as dlibc::pid_t && errno() as dlibc::c_int == dlibc::EBADF {
    ...
    // child will either exec() or invoke dlibc::exit)
    ...
    unsafe fn do_exec(
        ...
        if let Some(fd) = stdio.stdin.fd() {
            cvt_r(|| dlibc::dup2(fd, dlibc::STDIN_FILENO))?;
        }
        if let Some(fd) = stdio.stdout.fd() {
            cvt_r(|| dlibc::dup2(fd, dlibc::STDOUT_FILENO))?;
        }
        if let Some(fd) = stdio.stderr.fd() {
            cvt_r(|| dlibc::dup2(fd, dlibc::STDERR_FILENO))?;
        }
        ...
        #[cfg(not(target_os = "l4re"))]
        {
            if let Some(_g) = self.get_groups() {
                ...
                cvt(dlibc::setgroups(_g.len().try_into().unwrap(), _g.as_ptr()))?;
            }
            if let Some(u) = self.get_gid() {
                cvt(dlibc::setgid(u as gid_t))?;
            }
            if let Some(u) = self.get_uid() {
                ...
                //FIXME: Redox kernel does not support setgroups yet
                #[cfg(not(target_os = "redox"))]
                if dlibc::getuid() == 0 && self.get_groups().is_none() {
                    cvt(dlibc::setgroups(0, crate::std::ptr::null()))?;
                }
                cvt(dlibc::setuid(u as uid_t))?;
            }
        }
        if let Some(ref cwd) = *self.get_cwd() {
            cvt(dlibc::chdir(cwd.as_ptr()))?;
        }

        if let Some(pgroup) = self.get_pgroup() {
            cvt(dlibc::setpgid(0, pgroup))?;
        }
        ...
        #[cfg(not(target_os = "emscripten"))]
        {
            ...
            if !crate::std::sys::unix_sigpipe_attr_specified() {
        ...
        if let Some(envp) = maybe_envp {
            struct Reset(*const *const dlibc::c_char);
        ...
        dlibc::execvp(self.get_program_cstr().as_ptr(), self.get_argv().as_ptr());
    ...
    fn posix_spawn(
        ...
        use crate::std::sys::{self, cvt_nz, unix_sigpipe_attr_specified};
        ...
        unsafe fn retrying_libc_posix_spawnp(
            ...
            let mut delay = MIN_FORKSPAWN_SLEEP;
            loop {
                match dlibc::posix_spawnp(pid, file, file_actions, attrp, argv, envp) {
                    dlibc::EBADF => {
                        if delay < get_clock_resolution() {
                            // We cannot sleep this short (it would be longer).
                            // Yield instead.
        ...
        weak! {
            fn posix_spawn_file_actions_addchdir_np(
                *mut dlibc::posix_spawn_file_actions_t,
                *const dlibc::c_char
            ) -> dlibc::c_int
        }
        ...
        struct PosixSpawnFileActions<'a>(&'a mut MaybeUninit<dlibc::posix_spawn_file_actions_t>);

        impl Drop for PosixSpawnFileActions<'_> {
            fn drop(&mut self) {
                unsafe {
                    dlibc::posix_spawn_file_actions_destroy(self.0.as_mut_ptr());
                }
            }
        }

        struct PosixSpawnattr<'a>(&'a mut MaybeUninit<dlibc::posix_spawnattr_t>);

        impl Drop for PosixSpawnattr<'_> {
            fn drop(&mut self) {
                unsafe {
                    dlibc::posix_spawnattr_destroy(self.0.as_mut_ptr());
                }
            }
        }
        ...
        unsafe {
            ...
            cvt_nz(dlibc::posix_spawnattr_init(attrs.as_mut_ptr()))?;
            ...
            if let Some(fd) = stdio.stdin.fd() {
                cvt_nz(dlibc::posix_spawn_file_actions_adddup2(
            ...
            if !unix_sigpipe_attr_specified() {
                let mut default_set = MaybeUninit::<dlibc::sigset_t>::uninit();
                cvt(sigemptyset(default_set.as_mut_ptr()))?;
                cvt(sigaddset(default_set.as_mut_ptr(), dlibc::SIGPIPE))?;
                cvt_nz(dlibc::posix_spawnattr_setsigdefault(
                    attrs.0.as_mut_ptr(),
                    default_set.as_ptr(),
                ))?;
    ...
    #[cfg(target_os = "linux")]
    fn send_pidfd(&self, sock: &crate::std::sys::net::Socket) {
        use crate::std::io::IoSlice;
        use crate::std::os::fd::RawFd;
        use crate::std::sys::cvt_r;
        use dlibc::{CMSG_DATA, CMSG_FIRSTHDR, CMSG_LEN, CMSG_SPACE, SCM_RIGHTS, SOL_SOCKET};

        unsafe {
            let child_pid = dlibc::getpid();
            // pidfd_open sets CLOEXEC by default
            let pidfd = dlibc::syscall(dlibc::SYS_pidfd_open, child_pid, 0);
            ...
            let mut msg: dlibc::msghdr = mem::zeroed();
            ...
            match cvt_r(|| dlibc::sendmsg(sock.as_raw(), &msg, 0)) {
                Ok(0) => {}
    ...
    #[cfg(target_os = "linux")]
    fn recv_pidfd(&self, sock: &crate::std::sys::net::Socket) -> pid_t {
        use crate::std::io::IoSliceMut;
        use crate::std::sys::cvt_r;
        ...
        unsafe {
            ...
            match cvt_r(|| dlibc::recvmsg(sock.as_raw(), &mut msg, 0)) {
                Err(_) => return -1,
                Ok(_) => {}
            }
...
impl Process {
    ...
    pub fn kill(&mut self) -> io::Result<()> {
        ...
        if self.status.is_some() {
            ...
            cvt(unsafe { dlibc::kill(self.pid, dlibc::SIGKILL) }).map(drop)
        }
    }
    ...
    pub fn wait(&mut self) -> io::Result<ExitStatus> {
        ...
        cvt_r(|| unsafe { dlibc::waitpid(self.pid, &mut status, 0) })?;
    ...
    pub fn try_wait(&mut self) -> io::Result<Option<ExitStatus>> {
        ...
        let mut status = 0 as c_int;
        let pid = cvt(unsafe { dlibc::waitpid(self.pid, &mut status, dlibc::WNOHANG) })?;
        if pid == 0 {
            Ok(None)
        } else {
            self.status = Some(ExitStatus::new(status));
...
impl ExitStatus {
    ...
    pub fn code(&self) -> Option<i32> {
        self.exited().then(|| dlibc::WEXITSTATUS(self.0))
    }

    pub fn signal(&self) -> Option<i32> {
        dlibc::WIFSIGNALED(self.0).then(|| dlibc::WTERMSIG(self.0))
    }

    pub fn core_dumped(&self) -> bool {
        dlibc::WIFSIGNALED(self.0) && dlibc::WCOREDUMP(self.0)
    }

    pub fn stopped_signal(&self) -> Option<i32> {
        dlibc::WIFSTOPPED(self.0).then(|| dlibc::WSTOPSIG(self.0))
    }

    pub fn continued(&self) -> bool {
        dlibc::WIFCONTINUED(self.0)
    }
...
fn signal_string(signal: i32) -> &'static str {
    match signal {
        dlibc::SIGHUP => " (SIGHUP)",
        dlibc::SIGINT => " (SIGINT)",
        dlibc::SIGQUIT => " (SIGQUIT)",
        dlibc::SIGILL => " (SIGILL)",
        dlibc::SIGTRAP => " (SIGTRAP)",
        dlibc::SIGABRT => " (SIGABRT)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGBUS => " (SIGBUS)",
        dlibc::SIGFPE => " (SIGFPE)",
        dlibc::SIGKILL => " (SIGKILL)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGUSR1 => " (SIGUSR1)",
        dlibc::SIGSEGV => " (SIGSEGV)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGUSR2 => " (SIGUSR2)",
        dlibc::SIGPIPE => " (SIGPIPE)",
        dlibc::SIGALRM => " (SIGALRM)",
        dlibc::SIGTERM => " (SIGTERM)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGCHLD => " (SIGCHLD)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGCONT => " (SIGCONT)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGSTOP => " (SIGSTOP)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGTSTP => " (SIGTSTP)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGTTIN => " (SIGTTIN)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGTTOU => " (SIGTTOU)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGURG => " (SIGURG)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGXCPU => " (SIGXCPU)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGXFSZ => " (SIGXFSZ)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGVTALRM => " (SIGVTALRM)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGPROF => " (SIGPROF)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGWINCH => " (SIGWINCH)",
        #[cfg(not(any(target_os = "haiku", target_os = "l4re")))]
        dlibc::SIGIO => " (SIGIO)",
        #[cfg(target_os = "haiku")]
        dlibc::SIGPOLL => " (SIGPOLL)",
        #[cfg(not(target_os = "l4re"))]
        dlibc::SIGSYS => " (SIGSYS)",
...
#[cfg(target_os = "linux")]
impl crate::std::os::linux::process::ChildExt for crate::std::process::Child {
