...
use crate::std::sys::pipe::{self, AnonPipe};
use crate::std::sys_common::process::{CommandEnv, CommandEnvs};
use crate::std::sys_common::IntoInner;
use dlibc;

#[cfg(not(target_os = "fuchsia"))]
use crate::std::sys::fs::OpenOptions;

use dlibc::{c_char, c_int, gid_t, pid_t, uid_t, EXIT_FAILURE, EXIT_SUCCESS};
...
cfg_if::cfg_if! {
    if #[cfg(target_os = "android")] {
        #[allow(dead_code)]
        pub unsafe fn sigemptyset(set: *mut dlibc::sigset_t) -> dlibc::c_int {
            set.write_bytes(0u8, 1);
            return 0;
        }

        #[allow(dead_code)]
        pub unsafe fn sigaddset(set: *mut dlibc::sigset_t, signum: dlibc::c_int) -> dlibc::c_int {
            use crate::std::{
                mem::{align_of, size_of},
                slice,
            };
            use dlibc::{c_ulong, sigset_t};
            ...
            if set.is_null() || bit >= (8 * size_of::<sigset_t>()) {
                crate::std::sys::unix::os::set_errno(dlibc::EINVAL);
                return -1;
            }
        ...
        pub use dlibc::{sigemptyset, sigaddset};
    }
}
...
impl Stdio {
    pub fn to_child_stdio(&self, readable: bool) -> io::Result<(ChildStdio, Option<AnonPipe>)> {
        match *self {
            ...
            Stdio::Fd(ref fd) => {
                if fd.as_raw_fd() >= 0 && fd.as_raw_fd() <= dlibc::STDERR_FILENO {
...
pub struct CommandArgs<'a> {
    iter: crate::std::slice::Iter<'a, CString>,
}
