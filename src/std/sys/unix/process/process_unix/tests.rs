use crate::std::os::unix::process::{CommandExt, ExitStatusExt};
use crate::std::panic::catch_unwind;
...
fn test_command_fork_no_unwind() {
    ...
    assert!(
        signal == dlibc::SIGABRT
            || signal == dlibc::SIGILL
            || signal == dlibc::SIGTRAP
            || signal == dlibc::SIGSEGV
    );
}
...
fn test_command_pidfd() {
    ...
    use crate::std::os::linux::process::{ChildExt, CommandExt};
    use crate::std::process::Command;

    let our_pid = crate::std::process::id();
    let pidfd = unsafe { dlibc::syscall(dlibc::SYS_pidfd_open, our_pid, 0) };
    let pidfd_open_available = if pidfd >= 0 {
        unsafe { dlibc::close(pidfd as RawFd) };
        true
    } else {
        false
    };
