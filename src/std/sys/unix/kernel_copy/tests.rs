...
use crate::std::io::SeekFrom;
use crate::std::io::{BufRead, Read, Seek, Write};
...
fn copy_specialization() -> Result<()> {
    use crate::std::io::{BufReader, BufWriter};
    ...
    let result: Result<()> = try {
        let mut source = crate::std::fs::OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .truncate(true)
        ...
        let mut sink = crate::std::fs::OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .truncate(true)
    ...
    let rm1 = crate::std::fs::remove_file(source_path);
    let rm2 = crate::std::fs::remove_file(sink_path);
...
fn dont_splice_pipes_from_files() -> Result<()> {
    ...
    use crate::std::process::{ChildStdin, ChildStdout};
    use crate::std::sys_common::FromInner;

    let (read_end, write_end) = crate::std::sys::pipe::anon_pipe()?;
...
fn bench_file_to_file_copy(b: &mut test::Bencher) {
    ...
    let mut src = crate::std::fs::OpenOptions::new()
        .create(true)
        .truncate(true)
        .read(true)
        .write(true)
...
fn bench_file_to_socket_copy(b: &mut test::Bencher) {
    ...
    let sink_drainer = crate::std::net::TcpListener::bind("localhost:0").unwrap();
    let mut sink = crate::std::net::TcpStream::connect(sink_drainer.local_addr().unwrap()).unwrap();
...
fn bench_file_to_uds_copy(b: &mut test::Bencher) {
    ...
    let (mut sink, mut sink_drainer) = crate::std::os::unix::net::UnixStream::pair().unwrap();
...
fn bench_socket_pipe_socket_copy(b: &mut test::Bencher) {
    ...
    let (read_end, write_end) = crate::std::sys::pipe::anon_pipe().unwrap();
