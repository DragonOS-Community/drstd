...
pub unsafe fn init(argc: isize, argv: *const *const u8, sigpipe: u8) {
    ...
    unsafe fn sanitize_standard_fds() {
        ...
        'poll: {
            ...
            for pfd in pfds {
                if pfd.revents & dlibc::POLLNVAL == 0 {
                    continue;
                }
                if open64("/dev/null\0".as_ptr().cast(), dlibc::O_RDWR, 0) == -1 {
        ...
        )))]
        {
            ...
            use dlibc::open64;
            for fd in 0..3 {
                if dlibc::fcntl(fd, dlibc::F_GETFD) == -1 && errno() == dlibc::EBADF {
                    if open64("/dev/null\0".as_ptr().cast(), dlibc::O_RDWR, 0) == -1 {
                        // If the stream is closed but we failed to reopen it, abort the
                        // process. Otherwise we wouldn't preserve the safety of
                        // operations on the corresponding Rust object Stdin, Stdout, or
    ...
    unsafe fn reset_sigpipe(#[allow(unused_variables)] sigpipe: u8) {
        ...
        )))]
        {
            ...
            let (sigpipe_attr_specified, handler) = match sigpipe {
                sigpipe::DEFAULT => (false, Some(dlibc::SIG_IGN)),
                sigpipe::INHERIT => (true, None),
                sigpipe::SIG_IGN => (true, Some(dlibc::SIG_IGN)),
                sigpipe::SIG_DFL => (true, Some(dlibc::SIG_DFL)),
                _ => unreachable!(),
            };
...
static UNIX_SIGPIPE_ATTR_SPECIFIED: crate::std::sync::atomic::AtomicBool =
    crate::std::sync::atomic::AtomicBool::new(false);
...
pub(crate) fn unix_sigpipe_attr_specified() -> bool {
    UNIX_SIGPIPE_ATTR_SPECIFIED.load(crate::std::sync::atomic::Ordering::Relaxed)
}
...
pub use crate::std::sys::android::signal;
#[cfg(not(target_os = "android"))]
pub use dlibc::signal;
...
pub fn decode_error_kind(errno: i32) -> ErrorKind {
    use ErrorKind::*;
    match errno as dlibc::c_int {
        dlibc::E2BIG => ArgumentListTooLong,
        dlibc::EADDRINUSE => AddrInUse,
        dlibc::EADDRNOTAVAIL => AddrNotAvailable,
        dlibc::EBUSY => ResourceBusy,
        dlibc::ECONNABORTED => ConnectionAborted,
        dlibc::ECONNREFUSED => ConnectionRefused,
        dlibc::ECONNRESET => ConnectionReset,
        dlibc::EDEADLK => Deadlock,
        dlibc::EDQUOT => FilesystemQuotaExceeded,
        dlibc::EEXIST => AlreadyExists,
        dlibc::EFBIG => FileTooLarge,
        dlibc::EHOSTUNREACH => HostUnreachable,
        dlibc::EINTR => Interrupted,
        dlibc::EINVAL => InvalidInput,
        dlibc::EISDIR => IsADirectory,
        dlibc::ELOOP => FilesystemLoop,
        dlibc::ENOENT => NotFound,
        dlibc::ENOMEM => OutOfMemory,
        dlibc::ENOSPC => StorageFull,
        dlibc::ENOSYS => Unsupported,
        dlibc::EMLINK => TooManyLinks,
        dlibc::ENAMETOOLONG => InvalidFilename,
        dlibc::ENETDOWN => NetworkDown,
        dlibc::ENETUNREACH => NetworkUnreachable,
        dlibc::ENOTCONN => NotConnected,
        dlibc::ENOTDIR => NotADirectory,
        dlibc::ENOTEMPTY => DirectoryNotEmpty,
        dlibc::EPIPE => BrokenPipe,
        dlibc::EROFS => ReadOnlyFilesystem,
        dlibc::ESPIPE => NotSeekable,
        dlibc::ESTALE => StaleNetworkFileHandle,
        dlibc::ETIMEDOUT => TimedOut,
        dlibc::ETXTBSY => ExecutableFileBusy,
        dlibc::EXDEV => CrossesDevices,

        dlibc::EACCES | dlibc::EPERM => PermissionDenied,

        // These two constants can have the same value on some systems,
        // but different values on others, so we can't use a match
        // clause
        x if x == dlibc::EAGAIN || x == dlibc::EWOULDBLOCK => WouldBlock,
...
//
// There is confusion about whether dlibc::abort() flushes stdio streams.
// dlibc::abort() is required by ISO C 99 (7.14.1.1p5) to be async-signal-safe,
...
// stdlib doesn't use dlibc stdio buffering.  In a typical Rust program, which
// does not use C stdio, even a buggy dlibc::abort() is, in fact, safe.
