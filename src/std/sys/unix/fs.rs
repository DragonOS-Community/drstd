...
use crate::std::fmt;
use crate::std::io::{self, BorrowedCursor, Error, IoSlice, IoSliceMut, SeekFrom};
use crate::std::mem;
use crate::std::os::unix::io::{AsFd, AsRawFd, BorrowedFd, FromRawFd, IntoRawFd};
...
use crate::std::sys::time::SystemTime;
...
use dlibc::readdir_r as readdir64_r;
...
macro_rules! cfg_has_statx {
    ({ $($then_tt:tt)* } else { $($else_tt:tt)* }) => {
        cfg_if::cfg_if! {
            if #[cfg(all(target_os = "linux", target_env = "gnu",target_os = "dragonos",))] {
                $($then_tt)*
            } else {
                $($else_tt)*
            }
    ...
    ($($block_inner:tt)*) => {
        #[cfg(all(target_os = "linux", target_env = "gnu",target_os = "dragonos",))]
        {
            $($block_inner)*
        }
    };
...
cfg_has_statx! {{
    ...
    struct StatxExtraFields {
        ...
        stx_mask: u32,
        stx_btime: dlibc::statx_timestamp,
        // With statx, we can overcome 32-bit `time_t` too.
        #[cfg(target_pointer_width = "32")]
        stx_atime: dlibc::statx_timestamp,
        #[cfg(target_pointer_width = "32")]
        stx_ctime: dlibc::statx_timestamp,
        #[cfg(target_pointer_width = "32")]
        stx_mtime: dlibc::statx_timestamp,
    ...
    unsafe fn try_statx(
        ...
        if let Err(err) = cvt(statx(fd, path, flags, mask, &mut buf)) {
            ...
            let err2 = cvt(statx(0, ptr::null(), 0, dlibc::STATX_ALL, ptr::null_mut()))
                .err()
        ...
        stat.st_dev = dlibc::makedev(buf.stx_dev_major, buf.stx_dev_minor) as _;
        stat.st_ino = buf.stx_ino as dlibc::ino64_t;
        stat.st_nlink = buf.stx_nlink as dlibc::nlink_t;
        stat.st_mode = buf.stx_mode as dlibc::mode_t;
        stat.st_uid = buf.stx_uid as dlibc::uid_t;
        stat.st_gid = buf.stx_gid as dlibc::gid_t;
        stat.st_rdev = dlibc::makedev(buf.stx_rdev_major, buf.stx_rdev_minor) as _;
        stat.st_size = buf.stx_size as off64_t;
        stat.st_blksize = buf.stx_blksize as dlibc::blksize_t;
        stat.st_blocks = buf.stx_blocks as dlibc::blkcnt64_t;
        stat.st_atime = buf.stx_atime.tv_sec as dlibc::time_t;
        // `i64` on gnu-x86_64-x32, `c_ulong` otherwise.
        stat.st_atime_nsec = buf.stx_atime.tv_nsec as _;
        stat.st_mtime = buf.stx_mtime.tv_sec as dlibc::time_t;
        stat.st_mtime_nsec = buf.stx_mtime.tv_nsec as _;
        stat.st_ctime = buf.stx_ctime.tv_sec as dlibc::time_t;
...
cfg_has_statx! {{
    impl FileAttr {
        ...
        #[cfg(target_pointer_width = "32")]
        pub fn stx_mtime(&self) -> Option<&dlibc::statx_timestamp> {
            if let Some(ext) = &self.statx_extra_fields {
                if (ext.stx_mask & dlibc::STATX_MTIME) != 0 {
        ...
        #[cfg(target_pointer_width = "32")]
        pub fn stx_atime(&self) -> Option<&dlibc::statx_timestamp> {
            if let Some(ext) = &self.statx_extra_fields {
                if (ext.stx_mask & dlibc::STATX_ATIME) != 0 {
        ...
        #[cfg(target_pointer_width = "32")]
        pub fn stx_ctime(&self) -> Option<&dlibc::statx_timestamp> {
            if let Some(ext) = &self.statx_extra_fields {
                if (ext.stx_mask & dlibc::STATX_CTIME) != 0 {
...
impl FileAttr {
    ...
    pub fn created(&self) -> io::Result<SystemTime> {
        cfg_has_statx! {
            if let Some(ext) = &self.statx_extra_fields {
                return if (ext.stx_mask & dlibc::STATX_BTIME) != 0 {
...
impl FileType {
    pub fn is_dir(&self) -> bool {
        self.is(dlibc::S_IFDIR)
    }
    pub fn is_file(&self) -> bool {
        self.is(dlibc::S_IFREG)
    }
    pub fn is_symlink(&self) -> bool {
        self.is(dlibc::S_IFLNK)
    }
...
impl Drop for Dir {
    fn drop(&mut self) {
        let r = unsafe { dlibc::closedir(self.0) };
        assert!(
            r == 0 || crate::std::io::Error::last_os_error().is_interrupted(),
            "unexpected error during closedir: {:?}",
            crate::std::io::Error::last_os_error()
        );
    }
}
    ...
    pub fn metadata(&self) -> io::Result<FileAttr> {
        ...
        cfg_has_statx! {
            if let Some(ret) = unsafe { try_statx(
                fd,
                name,
                dlibc::AT_SYMLINK_NOFOLLOW | dlibc::AT_STATX_SYNC_AS_STAT,
                dlibc::STATX_ALL,
            ) } {
                return ret;
            }
        ...
        cvt(unsafe { fstatat64(fd, name, &mut stat, dlibc::AT_SYMLINK_NOFOLLOW) })?;
...
impl OpenOptions {
    ...
    fn get_access_mode(&self) -> io::Result<c_int> {
        match (self.read, self.write, self.append) {
            (true, false, false) => Ok(dlibc::O_RDONLY),
            (false, true, false) => Ok(dlibc::O_WRONLY),
            (true, true, false) => Ok(dlibc::O_RDWR),
            (false, _, true) => Ok(dlibc::O_WRONLY | dlibc::O_APPEND),
            (true, _, true) => Ok(dlibc::O_RDWR | dlibc::O_APPEND),
            (false, false, false) => Err(Error::from_raw_os_error(dlibc::EINVAL)),
        }
    }
    ...
    fn get_creation_mode(&self) -> io::Result<c_int> {
        match (self.write, self.append) {
            ...
            (false, false) => {
                if self.truncate || self.create || self.create_new {
                    return Err(Error::from_raw_os_error(dlibc::EINVAL));
                }
            }
            (_, true) => {
        ...
        Ok(match (self.create, self.truncate, self.create_new) {
            (false, false, false) => 0,
            (true, false, false) => dlibc::O_CREAT,
            (false, true, false) => dlibc::O_TRUNC,
            (true, true, false) => dlibc::O_CREAT | dlibc::O_TRUNC,
...
impl File {
    ...
    pub fn open_c(path: &CStr, opts: &OpenOptions) -> io::Result<File> {
        let flags = dlibc::O_CLOEXEC
            | opts.get_access_mode()?
            | opts.get_creation_mode()?
            | (opts.custom_flags as c_int & !dlibc::O_ACCMODE);
    ...
    pub fn file_attr(&self) -> io::Result<FileAttr> {
        ...
        cfg_has_statx! {
            if let Some(ret) = unsafe { try_statx(
                ...
                dlibc::AT_EMPTY_PATH | dlibc::AT_STATX_SYNC_AS_STAT,
                dlibc::STATX_ALL,
            ) } {
                return ret;
            }
    ...
    pub fn fsync(&self) -> io::Result<()> {
        ...
        unsafe fn os_fsync(fd: c_int) -> c_int {
            dlibc::fcntl(fd, dlibc::F_FULLFSYNC)
        }
        #[cfg(not(any(
            target_os = "macos",
            target_os = "ios",
    ...
    pub fn seek(&self, pos: SeekFrom) -> io::Result<u64> {
        let (whence, pos) = match pos {
            ...
            SeekFrom::Start(off) => (dlibc::SEEK_SET, off as i64),
            SeekFrom::End(off) => (dlibc::SEEK_END, off),
            SeekFrom::Current(off) => (dlibc::SEEK_CUR, off),
        };
    ...
    pub fn set_permissions(&self, perm: FilePermissions) -> io::Result<()> {
        cvt_r(|| unsafe { dlibc::fchmod(self.as_raw_fd(), perm.mode) })?;
        Ok(())
    }
    ...
    pub fn set_times(&self, times: FileTimes) -> io::Result<()> {
        ...
        let to_timespec = |time: Option<SystemTime>| {
            match time {
                ...
                Some(time) if time > crate::std::sys::time::UNIX_EPOCH => Err(io::const_io_error!(io::ErrorKind::InvalidInput, "timestamp is too large to set as a file time")),
                Some(_) => Err(io::const_io_error!(io::ErrorKind::InvalidInput, "timestamp is too small to set as a file time")),
                None => Ok(dlibc::timespec { tv_sec: 0, tv_nsec: dlibc::UTIME_OMIT as _ }),
            }
        };
        cfg_if::cfg_if! {
            if #[cfg(any(target_os = "redox", target_os = "espidf", target_os = "horizon"))] {
                ...
                let mut buf = [mem::MaybeUninit::<dlibc::timespec>::uninit(); 3];
                let mut num_times = 0;
                let mut attrlist: dlibc::attrlist = unsafe { mem::zeroed() };
                attrlist.bitmapcount = dlibc::ATTR_BIT_MAP_COUNT;
                if times.created.is_some() {
                    buf[num_times].write(to_timespec(times.created)?);
                    num_times += 1;
                    attrlist.commonattr |= dlibc::ATTR_CMN_CRTIME;
                }
                if times.modified.is_some() {
                    buf[num_times].write(to_timespec(times.modified)?);
                    num_times += 1;
                    attrlist.commonattr |= dlibc::ATTR_CMN_MODTIME;
                }
                if times.accessed.is_some() {
                    buf[num_times].write(to_timespec(times.accessed)?);
                    num_times += 1;
                    attrlist.commonattr |= dlibc::ATTR_CMN_ACCTIME;
                }
                cvt(unsafe { dlibc::fsetattrlist(
                    self.as_raw_fd(),
                    (&attrlist as *const dlibc::attrlist).cast::<dlibc::c_void>().cast_mut(),
                    buf.as_ptr().cast::<dlibc::c_void>().cast_mut(),
                    num_times * mem::size_of::<dlibc::timespec>(),
                    0
                ) })?;
                Ok(())
                ...
                cvt(unsafe {
                    weak!(fn futimens(c_int, *const dlibc::timespec) -> c_int);
                    match futimens.get() {
                ...
                {
                    use crate::std::sys::{time::__timespec64, weak::weak};

                    // Added in glibc 2.34
                    weak!(fn __futimens64(dlibc::c_int, *const __timespec64) -> dlibc::c_int);

                    if let Some(futimens64) = __futimens64.get() {
                        let to_timespec = |time: Option<SystemTime>| time.map(|time| time.t.to_timespec64())
                            .unwrap_or(__timespec64::new(0, dlibc::UTIME_OMIT as _));
                        let times = [to_timespec(times.accessed), to_timespec(times.modified)];
                ...
                let times = [to_timespec(times.accessed)?, to_timespec(times.modified)?];
                cvt(unsafe { dlibc::futimens(self.as_raw_fd(), times.as_ptr()) })?;
                Ok(())
            }
        }
    }
...
impl fmt::Debug for File {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        fn get_path(fd: c_int) -> Option<PathBuf> {
            ...
            // instead.
            let mut buf = vec![0; dlibc::PATH_MAX as usize];
            let n = unsafe { dlibc::fcntl(fd, dlibc::F_GETPATH, buf.as_ptr()) };
            if n == -1 {
                return None;
            }
        ...
        fn get_path(fd: c_int) -> Option<PathBuf> {
            let info = Box::<dlibc::kinfo_file>::new_zeroed();
            let mut info = unsafe { info.assume_init() };
            info.kf_structsize = mem::size_of::<dlibc::kinfo_file>() as dlibc::c_int;
            let n = unsafe { dlibc::fcntl(fd, dlibc::F_KINFO, &mut *info) };
            if n == -1 {
                return None;
            }
            let buf = unsafe {
        ...
        fn get_path(fd: c_int) -> Option<PathBuf> {
            let mut buf = vec![0; dlibc::PATH_MAX as usize];
            let n = unsafe { dlibc::ioctl(fd, dlibc::FIOGETNAME, buf.as_ptr()) };
            if n == -1 {
                return None;
            }
            let l = buf.iter().position(|&c| c == 0).unwrap();
        ...
        fn get_mode(fd: c_int) -> Option<(bool, bool)> {
            let mode = unsafe { dlibc::fcntl(fd, dlibc::F_GETFL) };
            if mode == -1 {
                return None;
            }
            match mode & dlibc::O_ACCMODE {
                dlibc::O_RDONLY => Some((true, false)),
                dlibc::O_RDWR => Some((true, true)),
                dlibc::O_WRONLY => Some((false, true)),
                _ => None,
            }
        }
...
pub fn readdir(path: &Path) -> io::Result<ReadDir> {
    let ptr = run_path_with_cstr(path, |p| unsafe { Ok(dlibc::opendir(p.as_ptr())) })?;
    if ptr.is_null() {
...
pub fn unlink(p: &Path) -> io::Result<()> {
    run_path_with_cstr(p, |p| cvt(unsafe { dlibc::unlink(p.as_ptr()) }).map(|_| ()))
}

pub fn rename(old: &Path, new: &Path) -> io::Result<()> {
    run_path_with_cstr(old, |old| {
        run_path_with_cstr(new, |new| {
            cvt(unsafe { dlibc::rename(old.as_ptr(), new.as_ptr()) }).map(|_| ())
        })
    })
}
...
pub fn rmdir(p: &Path) -> io::Result<()> {
    run_path_with_cstr(p, |p| cvt(unsafe { dlibc::rmdir(p.as_ptr()) }).map(|_| ()))
}
...
pub fn symlink(original: &Path, link: &Path) -> io::Result<()> {
    run_path_with_cstr(original, |original| {
        run_path_with_cstr(link, |link| {
            cvt(unsafe { dlibc::symlink(original.as_ptr(), link.as_ptr()) }).map(|_| ())
        })
    })
}
        ...
        run_path_with_cstr(link, |link| {
            cfg_if::cfg_if! {
                if #[cfg(any(target_os = "vxworks", target_os = "redox", target_os = "android", target_os = "espidf", target_os = "horizon", target_os = "vita"))] {
                    ...
                    cvt(unsafe { dlibc::link(original.as_ptr(), link.as_ptr()) })?;
                    ...
                    if let Some(f) = linkat.get() {
                        cvt(unsafe { f(dlibc::AT_FDCWD, original.as_ptr(), dlibc::AT_FDCWD, link.as_ptr(), 0) })?;
                    } else {
                        cvt(unsafe { dlibc::link(original.as_ptr(), link.as_ptr()) })?;
                    };
                } else {
                    ...
                    cvt(unsafe { dlibc::linkat(dlibc::AT_FDCWD, original.as_ptr(), dlibc::AT_FDCWD, link.as_ptr(), 0) })?;
                }
            }
            Ok(())
        })
...
pub fn canonicalize(p: &Path) -> io::Result<PathBuf> {
    let r = run_path_with_cstr(p, |path| unsafe {
        Ok(dlibc::realpath(path.as_ptr(), ptr::null_mut()))
    })?;
    if r.is_null() {
    ...
    Ok(PathBuf::from(OsString::from_vec(unsafe {
        ...
        dlibc::free(r as *mut _);
        buf
    })))
}

fn open_from(from: &Path) -> io::Result<(crate::std::fs::File, crate::std::fs::Metadata)> {
    use crate::std::fs::File;
    use crate::std::sys_common::fs::NOT_FILE_ERROR;
...
fn open_to_and_set_permissions(
    to: &Path,
    reader_metadata: crate::std::fs::Metadata,
) -> io::Result<(crate::std::fs::File, crate::std::fs::Metadata)> {
...
fn open_to_and_set_permissions(
    ...
    use crate::std::fs::OpenOptions;
    use crate::std::os::unix::fs::{OpenOptionsExt, PermissionsExt};
...
pub fn copy(from: &Path, to: &Path) -> io::Result<u64> {
    ...
    type copyfile_state_t = *mut dlibc::c_void;
    #[allow(non_camel_case_types)]
    ...
    extern "C" {
        ...
        fn copyfile_state_free(state: copyfile_state_t) -> dlibc::c_int;
        fn copyfile_state_get(
            state: copyfile_state_t,
            flag: u32,
            dst: *mut dlibc::c_void,
        ) -> dlibc::c_int;
    }
    ...
    if HAS_FCLONEFILEAT.load(Ordering::Relaxed) {
        let clonefile_result = run_path_with_cstr(to, |to| {
            cvt(unsafe { fclonefileat(reader.as_raw_fd(), dlibc::AT_FDCWD, to.as_ptr(), 0) })
        });
        match clonefile_result {
            ...
            Err(err) => match err.raw_os_error() {
                ...
                // should succeed.
                Some(dlibc::ENOTSUP) | Some(dlibc::EEXIST) | Some(dlibc::EXDEV) => (),
                Some(dlibc::ENOSYS) => HAS_FCLONEFILEAT.store(false, Ordering::Relaxed),
                _ => return Err(err),
            },
        }
    }
    ...
    let state = unsafe {
        ...
        if state.is_null() {
            return Err(crate::std::io::Error::last_os_error());
        }
        FreeOnDrop(state)
    };
    ...
    let mut bytes_copied: dlibc::off_t = 0;
    cvt(unsafe {
        copyfile_state_get(
            state.0,
            COPYFILE_STATE_COPIED,
            &mut bytes_copied as *mut dlibc::off_t as *mut dlibc::c_void,
        )
    })?;
...
pub fn chown(path: &Path, uid: u32, gid: u32) -> io::Result<()> {
    run_path_with_cstr(path, |path| {
        cvt(unsafe { dlibc::chown(path.as_ptr(), uid as dlibc::uid_t, gid as dlibc::gid_t) })
            .map(|_| ())
    })
}

pub fn fchown(fd: c_int, uid: u32, gid: u32) -> io::Result<()> {
    cvt(unsafe { dlibc::fchown(fd, uid as dlibc::uid_t, gid as dlibc::gid_t) })?;
    Ok(())
}

pub fn lchown(path: &Path, uid: u32, gid: u32) -> io::Result<()> {
    run_path_with_cstr(path, |path| {
        cvt(unsafe { dlibc::lchown(path.as_ptr(), uid as dlibc::uid_t, gid as dlibc::gid_t) })
            .map(|_| ())
    })
}
...
mod remove_dir_impl {
    pub use crate::std::sys_common::fs::remove_dir_all;
}
...
mod remove_dir_impl {
    ...
    use crate::std::io;
    use crate::std::os::unix::io::{AsRawFd, FromRawFd, IntoRawFd};
    use crate::std::os::unix::prelude::{OwnedFd, RawFd};
    use crate::std::path::{Path, PathBuf};
    use crate::std::sys::common::small_c_string::run_path_with_cstr;
    use crate::std::sys::{cvt, cvt_r};
    ...
    #[cfg(not(any(
        ...
        all(target_os = "dragonos")
    )))]
    use dlibc::{fdopendir, openat, unlinkat};
    #[cfg(all(target_os = "linux", target_env = "gnu"))]
    use dlibc::{fdopendir, openat64 as openat, unlinkat};
    ...
    pub fn openat_nofollow_dironly(parent_fd: Option<RawFd>, p: &CStr) -> io::Result<OwnedFd> {
        let fd = cvt_r(|| unsafe {
            openat(
                ...
                p.as_ptr(),
                dlibc::O_CLOEXEC | dlibc::O_RDONLY | dlibc::O_NOFOLLOW | dlibc::O_DIRECTORY,
            )
        })?;
    ...
    fn fdreaddir(dir_fd: OwnedFd) -> io::Result<(ReadDir, RawFd)> {
        ...
        let dirp = Dir(ptr);
        // file descriptor is automatically closed by dlibc::closedir() now, so give up ownership
    ...
    fn is_dir(ent: &DirEntry) -> Option<bool> {
        match ent.entry.d_type {
            dlibc::DT_UNKNOWN => None,
            dlibc::DT_DIR => Some(true),
            _ => Some(false),
        }
    }
    ...
    fn remove_dir_all_recursive(parent_fd: Option<RawFd>, path: &CStr) -> io::Result<()> {
        ...
        let fd = match openat_nofollow_dironly(parent_fd, &path) {
            Err(err) if matches!(err.raw_os_error(), Some(dlibc::ENOTDIR | dlibc::ELOOP)) => {
    ...
    pub fn remove_dir_all(p: &Path) -> io::Result<()> {
        if macos_weak::has_openat() {
            ...
            crate::std::sys_common::fs::remove_dir_all(p)
        }
    }
}
