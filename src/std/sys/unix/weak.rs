...
use crate::std::mem;
use crate::std::ptr;
use crate::std::sync::atomic::{self, AtomicPtr, Ordering};
use dlibc;
...
impl<F> DlsymWeak<F> {
    ...
    pub(crate) fn get(&self) -> Option<F> {
        unsafe {
            ...
            match self.func.load(Ordering::Relaxed) {
                ...
                func => {
                    let func = mem::transmute_copy::<*mut dlibc::c_void, F>(&func);
    ...
    unsafe fn initialize(&self) -> Option<F> {
        assert_eq!(mem::size_of::<F>(), mem::size_of::<*mut dlibc::c_void>());
...
unsafe fn fetch(name: &str) -> *mut dlibc::c_void {
    let name = match CStr::from_bytes_with_nul(name.as_bytes()) {
        Ok(cstr) => cstr,
        Err(..) => return ptr::null_mut(),
    };
    dlibc::dlsym(dlibc::RTLD_DEFAULT, name.as_ptr())
}
    ...
    (fn $name:ident($($arg_name:ident: $t:ty),*) -> $ret:ty) => (
        unsafe fn $name($($arg_name: $t),*) -> $ret {
            ...
            if let Some(fun) = $name.get() {
                ...
                super::os::set_errno(dlibc::ENOSYS);
                -1
            }
        }
    )
