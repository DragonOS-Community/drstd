...
pub unsafe extern "C" fn runtime_entry(
    ...
    use crate::std::sys::hermit::thread_local_dtor::run_dtors;
    extern "C" {
...
pub fn cvt<T: IsNegative>(t: T) -> crate::std::io::Result<T> {
    if t.is_negative() {
        let e = decode_error_kind(t.negate());
        Err(crate::std::io::Error::from(e))
    } else {
        Ok(t)
    }
}
