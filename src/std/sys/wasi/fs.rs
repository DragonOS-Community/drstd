...
use crate::std::os::wasi::ffi::{OsStrExt, OsStringExt};
use crate::std::os::wasi::io::{AsFd, AsRawFd, BorrowedFd, FromRawFd, IntoRawFd, RawFd};
    ...
    run_path_with_cstr(p, |p| {
        ...
        loop {
            unsafe {
                ...
                if fd == -1 {
                    if io::Error::last_os_error().raw_os_error() == Some(dlibc::ENOMEM) {
        ...
        extern "C" {
            pub fn __wasilibc_find_relpath(
                path: *const dlibc::c_char,
                abs_prefix: *mut *const dlibc::c_char,
                relative_path: *mut *const dlibc::c_char,
                relative_path_len: dlibc::size_t,
            ) -> dlibc::c_int;
        }
    })
}
