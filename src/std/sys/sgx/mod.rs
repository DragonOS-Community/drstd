...
pub fn unsupported<T>() -> crate::std::io::Result<T> {
    Err(unsupported_err())
}

pub fn unsupported_err() -> crate::std::io::Error {
    crate::std::io::const_io_error!(ErrorKind::Unsupported, "operation not supported on SGX yet")
}
...
pub fn sgx_ineffective<T>(v: T) -> crate::std::io::Result<T> {
...
pub mod rand {
    pub fn rdrand64() -> u64 {
        unsafe {
            let mut ret: u64 = 0;
            for _ in 0..10 {
                if crate::std::arch::x86_64::_rdrand64_step(&mut ret) == 1 {
                    return ret;
                }
            }
...
pub use crate::std::sys_common::{AsInner, FromInner, IntoInner};
